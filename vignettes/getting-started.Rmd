...
- shinyF7 is built on top of the [framework7](https://framework7.io) template. It may be 
...
- shinyF7 brings 3 out of the box layouts:

  * `f7SingleLayout`: develop simple shinyF7 Apps (best choice for iOS/android Apps)
...
- shinyF7 has its own custom input widgets with unique design for each skin (iOS/android/aurora):
...
- shinyF7 has custom input values for sidebar and tabs as well as dedicated update 
...
in case you want to display a loading screen. `f7Page()` accepts any shinyF7 layout.

##### Navbar
The navbar is one of the most important element of any shinyF7 layout.
...
theme is light, and conversely. Its behaviour is controlled via the style argument:
...
This will be probably included in a future release.
...
shinyF7 brings a lot of different colors. The "panelSwipeSide" let panels to be shifted
by hand. hideNavOnPageScroll and hideTabsOnPageScroll allow to hide/show the navbar and toolbar
which is useful to focus on the content. Note that the serviceWorker option is not working at the moment.

framework7 has more options which will be implemented in a next version of shinyF7.
